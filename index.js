...
app.post('/generate', async (req, res) => {
    try {
        ...
        const parsedJSON = aiResponse.data.choices[0].message.content;

        // parse the returned string into JSON (OpenAI response_format gives us clean JSON string)
        const lessonObject = JSON.parse(parsedJSON);
